...
public class OpenCommandTest {
	...
	public void should_Execute_Command_Correctly() {
		String[] args = { "chest" };
		EasyMock.expect(
				gameInstance.openContainer(args[OpenCommand.ITEM_INDEX]))
				.andReturn(true).times(1);
		EasyMock.replay(gameInstance);

		AbstractCommand command = injector.getInstance(OpenCommand.class);
		Assert.assertNotNull(command);
		boolean result = command.execute(new UserCommand("open", args));
	...
	public void should_Execute_Command_With_Wrong_Arguments_Correctly() {
		...
		AbstractCommand command = injector.getInstance(OpenCommand.class);
		Assert.assertNotNull(command);
		Assert.assertTrue(!command.execute(new UserCommand("open", args)));
		EasyMock.verify(gameInstance);
	}
	...
	public void should_Execute_Command_With_Null_Argument_Correctly() {
		String[] args = null;
		EasyMock.expect(gameInstance.openContainer(null)).andReturn(false)
				.times(1);
		EasyMock.replay(gameInstance);
